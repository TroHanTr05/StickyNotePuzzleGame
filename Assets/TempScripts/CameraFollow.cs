...
namespace Game.Runtime
{
    ...
    public class OrthoCameraFollow : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            if (snapToTargetOnStart) SnapInstant();
        }
        ...
        private void Update()
        {
            HandleZoom();
            cam.orthographic     = true;
            cam.orthographicSize = DampFloat(cam.orthographicSize, zoomTarget, zoomSharpness, DeltaTime);
        }
        ...
        {
            if (!allowZoom || Mouse.current == null) return;

            float scroll = Mouse.current.scroll.ReadValue().y;
            if (Mathf.Abs(scroll) > 0.01f)
                zoomTarget = Mathf.Clamp(zoomTarget - scroll * 0.01f * zoomSensitivity, minSize, maxSize);
        ...
        private void UpdateCameraPose()
        {
            ...
            smoothedLookAhead  = DampVector3(smoothedLookAhead, lookAhead, lookAheadSharpness, dt);
            Vector3 desiredFocus = rawFocusPoint + smoothedLookAhead;
            smoothedFocusPoint = DampVector3(smoothedFocusPoint, desiredFocus, focusSharpness, dt);

            Vector3 desiredPosition = smoothedFocusPoint + worldOffset;
            transform.position = DampVector3(transform.position, desiredPosition, followSharpness, dt);
        ...
        public void SetTarget(Transform newTarget, bool snapInstantly = true)
        {
            ...
            if (snapInstantly) SnapInstant();
        }

        public void SetZoom(float newSize)  => zoomTarget = Mathf.Clamp(newSize, minSize, maxSize);
        public void ResetZoom()             => zoomTarget = orthoSize;

        private static float ExpDampFactor(float sharpness, float dt) => 1f - Mathf.Exp(-sharpness * dt);
        private static float DampFloat(float current, float target, float sharpness, float dt)
            => Mathf.Lerp(current, target, ExpDampFactor(sharpness, dt));
        private static Vector3 DampVector3(Vector3 current, Vector3 target, float sharpness, float dt)
            => Vector3.Lerp(current, target, ExpDampFactor(sharpness, dt));
